    ...
    static {
        ...
        register(
                "bd_engine", context -> {
                    ...
                    final DisplayModel displayModel = BDEngine.parse(Holder.stringDataModel);
                    final DisplayModel displayModelText = BDEngine.parse(Holder.stringDataText);
        ...
        register(
                "bd_engine_multipart", context -> {
                    final DisplayModel model = BDEngine.parse(
                   ...
                   .then(SchedulerTask.await(promise -> {
                       new EternaRunnable(Eterna.getPlugin()) {
                           ...
                           public void run() {
                               ...
                               displayEntity.editRotation(edit -> {
                                   final float pitch = (float) (Math.toRadians(2) * Math.cos(Math.toRadians(Bukkit.getCurrentTick()) * 10));
                                   final float roll = (float) (Math.toRadians(2) * Math.sin(Math.toRadians(Bukkit.getCurrentTick()) * 10));
