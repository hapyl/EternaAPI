...
public final class QuestManager extends EternaManager<Player, QuestDataList> implements Listener {
        ...
        workHandlers(handler -> {
            ...
            handler.getQuests().forEach(quest -> {
                for (QuestStartBehaviour startBehaviour : quest.getStartBehaviours()) {
                    if (!(startBehaviour instanceof QuestStartBehaviour.OnJoin(boolean sendNotification))) {
                        continue;
                    }
                    
                    if (canStartQuest(player, quest, false).isOk()) {
                        startQuestForcefully(player, quest, sendNotification);
                    }
                }
            });
        });
        ...
        tryStartQuest(
                player, quest -> {
                    for (QuestStartBehaviour startBehaviour : quest.getStartBehaviours()) {
                        ...
                        // We don't start the quest because we have to finish the dialog
                        return false;
                    }
    ...
    public void handleAsyncPlayerChatEvent(AsyncChatEvent ev) {
        ...
        tryIncrementObjective(
                ev.getPlayer(), TalkInChatQuestObjective.class, (objective, response) -> {
    ...
    public void handleVehicleMoveEvent(PlayerMoveOneBlockEvent ev) {
        ...
        tryStartQuest(
                player, quest -> {
                    for (QuestStartBehaviour startBehaviour : quest.getStartBehaviours()) {
                        if (!(startBehaviour instanceof QuestStartBehaviour.GoTo(@Nonnull Position position, @Nonnull Dialog dialog))) {
                            continue;
                        }
                        ...
                        // If the dialog exists, don't start the quest right away, only after the dialog ends
                        if (dialog != null) {
                            dialog.start(player);
                            return false;
                        }
    ...
    public <T extends QuestObjective> void tryIncrementObjective(
            @Nonnull Player player,
            @Nonnull Class<T> clazz,
            @Nonnull BiConsumer<T, QuestObjective.Response> consumer,
    ...
    public void startQuestForcefully(@Nonnull Player player, @Nonnull Quest quest, boolean notifyStart) {
        final QuestData data = get(player).getDataOrCompute(quest);
        
        if (notifyStart) {
            quest.getFormatter().sendQuestStartedFormat(player, quest);
        }
        
        quest.onStart(player, data);
    }
