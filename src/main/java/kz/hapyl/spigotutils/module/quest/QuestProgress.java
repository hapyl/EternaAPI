...
public class QuestProgress {
	...
	public void nextObjective() {
		...
		if (completedObjective == null) {
			Chat.sendMessage(player, "&cThere was an error whilst trying to complete Quest objective! Quest rewards were saved and granted.");
	...
	private void sendQuestCompleteInfo(Player player) {
		...
		if (!this.isAutoClaimReward()) {
			Chat.sendCenterMessage(player, "&aUse &e/quest &ato claim rewards!");
		}
		Chat.sendCenterMessage(player, "");
		Chat.sendCenterMessage(player, "&e&m]                                      [");
	}
