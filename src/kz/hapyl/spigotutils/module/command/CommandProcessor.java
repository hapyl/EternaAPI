		...
		try {
			...
			for (final SimpleCommand cmd : array) {
				...
				if (simpleMap.getCommand(cmd.getName()) != null) {
					throw new IllegalArgumentException(String.format("Command %s already registered in %s!", cmd.getName(), this.plugin.getName()));
				}

				simpleMap.register(this.plugin.getName(),
