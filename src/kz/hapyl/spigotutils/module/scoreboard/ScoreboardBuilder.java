...
public class ScoreboardBuilder {
	...
	public ScoreboardBuilder(String scoreName, String displayName, int lines) {
		...
		if (lines > 17) {
			Bukkit.getLogger().warning("Creating scoreboard with more than 17 lines, not all lines will render client side!");
		}
	...
	public void setLines(String... lines) {
		...
		if (lines.length != this.scoreLines) {
			final ScoreboardBuilder builder = new ScoreboardBuilder(this.obj.getName(), this.obj.getDisplayName(), lines.length);
			this.scoreLines = builder.scoreLines;
			this.score = builder.score;
			this.obj = builder.obj;
		}
	...
	private ScoreboardBuilder rebuild(int add) {
		...
		if (result < 0 || result > ChatColor.ALL_CODES.length())
			throw new IndexOutOfBoundsException(
					"Cannot have less than 0 or more than %%line%% lines!".replace("%%line%%", ChatColor.ALL_CODES.length() + ""));
